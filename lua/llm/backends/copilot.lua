  ...
  require("plenary.curl").get(
    "https://api.github.com/copilot_internal/v2/token",
    {
      ...
      callback = function(data)
        local status, response = pcall(json.decode, data.body)

        if status then
          self.token = response.token
          self.expires_time = response.expires_at
        end
...
function copilot.ParseHandler(chunk, ctx)
...
function copilot.FunctionCalling(ctx, t)
  ...
  require("plenary.job")
    :new({
      ...
      on_stdout = schedule_wrap(function(_, c)
        if ctx.stream then
          ctx.assistant_output = copilot.StreamingHandler(c, ctx)
        else
          copilot.ParseHandler(c, ctx)
        end
      end),
...
function copilot.AppendToolsResponse(results, msg)
...
function copilot.GetToolsResponse(chunk, msg)
...
function copilot.StreamingTblHandler(results)
