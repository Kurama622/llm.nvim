...
function M.GetStreamingOutput(opts)
  ...
  local cb = function()
      ...
      on_exit = vim.schedule_wrap(function()
        if ctx.body.tools ~= nil and F.IsValid(backends.msg_tool_calls_content) then
          ctx.callback = function()
            exit_callback(opts, ctx, state.llm.worker)
          end
    ...
    state.llm.worker.job:start()

    -- return worker
  end
  for idx, cmd in ipairs(state.enabled_cmds) do
    opts.enable_cmds_idx = idx
    -- opts.cb = vim.schedule_wrap(cb)
    cmd.callback(conf.configs.web_search, opts.messages, opts)
    vim.schedule(cb)
