...
local function init(opts)
  if not state.completion.frontend then
    if opts.style == "virtual_text" then
      ...
      local virtual_text =
        require("llm.common.completion.frontends.virtual_text")
    ...
    if opts.style == "nvim-cmp" then
      local cond = opts.filetypes[vim.bo.ft] == nil
          and opts.default_filetype_enabled
        or opts.filetypes[vim.bo.ft]
      if cond then
